...
public class TheWildUpgrade implements ModInitializer {
	...
	private void upgradeBirchForests() {
		...
		birchForestModifier.add(ModificationPhase.ADDITIONS, ALL_BIRCH_FORESTS, context -> {
			context.getGenerationSettings().addFeature(GenerationStep.Feature.LOCAL_MODIFICATIONS, TheWildUpgradeFeatures.BIRCH_FOREST_ROCK.getKey().orElseThrow());
			...
			context.getGenerationSettings().addFeature(GenerationStep.Feature.VEGETAL_DECORATION, TheWildUpgradeFeatures.WHITE_VIOLETS_PLACED.getKey().orElseThrow());
